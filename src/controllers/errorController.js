...
errorController.get("/403" , (req , res , next) => {
    ...
    if (
      ...
      res
        .status(statusCode)
        .render(path.join(__dirname + "/../views/errors/auth/error"), {
          req: req,
          title: title,
      ...
      res
        .status(statusCode)
        .render(path.join(__dirname + "/../views/errors/guest/error"), {
          title: title,
          statusCode: statusCode,
          message: message,
        });
...
errorController.use((req, res) => {
    ...
    if (
      ...
      res.status(statusCode).render(path.join(__dirname + "/../views/errors/auth/error"), {
            req: req,
            title: title,
            statusCode: statusCode,
            message : message,
      ...
      res.status(statusCode).render(path.join(__dirname + "/../views/errors/guest/error"), {
        title: title,
        statusCode: statusCode,
        message: message,
      });
