...
function editUser(button) {
  ...
  var userId = rowData.userId || rowData.id;
  if (!userId) {
    alertMessage("Haujafanikiwa!", "Imeshindikana kupata kitambulisho cha mtumiaji.", "error", () => {});
    return;
  }
  $("#id-field").remove();
...
function getUser(userId) {
  ...
  ajaxRequest(
    ...
    "GET",
    (userResponse) => {
      if (userResponse.statusCode == 300 && Array.isArray(userResponse.data) && userResponse.data.length > 0) {
        ...
        alertMessage("Haujafanikiwa!", userResponse.message || "Taarifa za mtumiaji hazikupatikana.", "error", () => {});
      }
    },
    {},
    false
...
function activateDeactivateUser(button) {
  const rowData = JSON.parse(button.getAttribute("data-row"));
  var nameId = rowData.userId || rowData.id;
