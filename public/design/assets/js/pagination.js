...
function paginate(url , pages , current , per_page){
    ...
    if(pages > 1){
                ...
                var paginationDiv = '<div class="pagination-wrap hstack gap-2">';
                    paginationDiv =  paginationDiv +`<a class="page-item pagination-prev ${current == 1 ? 'disabled' : ''}"  href='${page==current ? '#' : url }'>First</a>` 
                    ...
                    paginationDiv = paginationDiv+`<a href='${current==pages ? '#' : url+'?page='+pages }'  class="page-item pagination-next ${current==pages ? 'disabled' : ''}">Last</a>` 
