(function () {
  ...
  const setCounter = (totalPending = 0) => {
    ...
    $("#unread-notification-counter").text(total);
    $("#all-noti-tab-count").text(`(${total})`);
    $("#all-new-messages-count").text(total > 0 ? `${total} mapya` : "0 mapya");
    $("#all-messages-tab-count").text(total > 0 ? `(${total})` : "");
  };
