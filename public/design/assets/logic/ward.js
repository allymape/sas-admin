...
function nata(){
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $(".caption").html(
          `<span>Idadi ya Kata zilizopatikana ${total}.</span> <span class='justify-content-end'>Ukurasa ${current}  kati ya ${pages}     [${first_item} hadi ${
            first_item - 1 + wards.length
          }] </span>`
        );
        for (var i = 0; i < wards.length; i++) {
          ...
          row =
            ...
            formatDate(wards[i].CreatedAt) +
            " </span></td>";
          row =
            row +
            '<td class="status"><span> ' +
            formatDate(wards[i].UpdatedAt) +
            "  </span></td>";
          row = row + "</tr>";
