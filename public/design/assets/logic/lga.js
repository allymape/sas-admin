...
function nata(){
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $(".caption").html(
          `<span>Idadi ya Halmashauri zilizopatikana ${total}.</span> <span class='justify-content-end'>Ukurasa ${current}  kati ya ${pages}     [${first_item} hadi ${
            first_item - 1 + councils.length
          }] </span>`
        );
        for (var i = 0; i < councils.length; i++) {
          ...
          row =
            ...
            councils[i].regionName +
            " </span></td>";
          row =
            row +
            '<td class="status"><span> ' +
            formatDate(councils[i].createdAt) +
            " </span></td>";
          row =
            row +
            '<td class="status"><span> ' +
            formatDate(councils[i].updatedAt) +
            " </span></td>";
          row = row + "</tr>";
