  ...
  sendRequest(
    ...
    (body) => {
      if (body !== undefined) {
        ...
        if (statusCode == 300) {
          res.render(path.join(__dirname + "/../design/uongozi"), {
            req: req,
...
hierarchyController.get("/hierarchies",  isAuthenticated, can('view-hierarchies'), function (req, res) {
    ...
    sendRequest(req, res, allHierarchiesAPI, "GET", formData, (jsonData) => {
...
function getAllHierarchies(req, res, edit = false, editedData = null) {
  ...
  sendRequest(req, res, url, "GET", formData, (jsonData) => {
     if (jsonData !== undefined) {
       ...
       var numRows = jsonData.numRows;
       if (statusCode == 300) {
         res.render(path.join(__dirname + "/../design/hierarchies"), {
           req: req,
           data: data,
