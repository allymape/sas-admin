...
algorithmController.get("/Algorithm", isAuthenticated, can('view-algorithm') , function (req, res) {
   
        sendRequest(req , res , categoriesAPI ,"GET" , {} , (jsonData) => {
            const { data } = jsonData;
            res.render(path.join(__dirname + "/../design/algorithm"), {
              req: req,
              categories: data,
            });
        } )
...
// Get all algorithm
algorithmController.get("/All-algorithms",  isAuthenticated, can('view-algorithm'),function (req, res) {
  ...
  var page = Number(req.query.page || 1);
    ...
    console.log("nipo")
    sendRequest(req, res, allalgorithmAPI+ "?page=" + page + "&per_page=" + per_page, "GET", formData, (jsonData) => {
...
// Store Algorithm
algorithmController.post("/TengenezaAlgorithm",  isAuthenticated, can('create-algorithm'), function (req, res) {
...
// Edit Algorithm
algorithmController.get("/Algorithm/:id",  isAuthenticated, can('update-algorithm'), function (req, res) {
  var id = Number(req.params.id);
  sendRequest(req, res, editAlgorithmAPI + "/" + id, "GET", {}, (jsonData) => {
      getAllAlgorithm(req, res, true, jsonData.data);
  });
});

// Update Algorithm
algorithmController.post("/BadiliAlgorithm/:id",  isAuthenticated, can('update-algorithm'), function (req, res) {
  var id = Number(req.params.id);
  sendRequest(req, res, updateAlgorithmAPI + "/" + id, "PUT", req.body , (jsonData) => {
...
// Delete Algorithm
algorithmController.post("/FutaAlgorithm/:id",  isAuthenticated, can('delete-algorithm'), function (req, res) {
  var id = Number(req.params.id);
  sendRequest(req, res, deleteAlgorithmAPI + "/" + id, "PUT", {}, (jsonData) => {
        var statusCode = jsonData.statusCode;
        var message = jsonData.message;
         res.send({
           statusCode: statusCode,
